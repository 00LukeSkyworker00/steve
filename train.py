...
parser.add_argument('--dvae_dir', type=str, default='results/dvae/')
...
parser.add_argument('--wandb_proj_sufix', type=str, default='')
...
parser.add_argument('--dvae_pretrain', default=False, action='store_true')
...
# Setup WanDB
wandb.login()
project_name = f'steve_pretrain_{ds_type}{args.wandb_proj_sufix}'
run = wandb.init(
...
if os.path.isfile(dvae_checkpoint_path):
    ...
    model = STEVE(args)
    dvae_state = torch.load(dvae_checkpoint_path, map_location='cpu', weights_only=True)
    model.dvae.load_state_dict(dvae_state)
    print(f'Loaded pretrained dVAE from {dvae_checkpoint_path}')
    for param in model.dvae.parameters():
        param.requires_grad = False
    print('dVAE parameters frozen')
    train_dvae = False
else:
    print(f'Pretrained dVAE not found at {dvae_checkpoint_path}, training dVAE')
    train_dvae = True
...
# Set model and ckpt name
if args.dvae_pretrain:
    state_dict = model.module.dvae.state_dict() if args.use_dp else model.dvae.state_dict()
    best_pth = dvae_checkpoint_path
    ckpt_pth =os.path.join(dvae_dir, 'dvae_checkpoint.pt.tar')
    os.makedirs(dvae_dir, exist_ok=True)
else:
    state_dict = model.module.state_dict() if args.use_dp else model.state_dict()
...
if train_dvae:
    params += [{'params': (x[1] for x in model.named_parameters() if 'dvae' in x[0]), 'lr': args.lr_dvae}]
...
for epoch in range(start_epoch, args.epochs):
    ...
    for batch, video in enumerate(train_loader):
        ...
        model_out = model(video, tau, args.hard)

        if args.dvae_pretrain:
            (recon, mse) = model_out
        else:
            (recon, cross_entropy, mse, attns) = model_out
            ...
            if batch % log_interval == 0:
                ...
                if not args.dvae_pretrain:
                    log_dict.update({
                        ...
                        "TRAIN/lr_dec": optimizer.param_groups[1]['lr'],
                    })
                    if train_dvae:
                        log_dict.update({
                            "TRAIN/lr_dvae": optimizer.param_groups[3]['lr'],
                        })
                else:
                    log_dict.update({
                ...
                if not args.dvae_pretrain:
                    ...
                    if train_dvae:
                        writer.add_scalar('TRAIN/lr_dvae', optimizer.param_groups[2]['lr'], global_step)
                else:
                    writer.add_scalar('TRAIN/lr_dvae', optimizer.param_groups[0]['lr'], global_step)
    ...
    with torch.no_grad():
        ...
        if args.dvae_pretrain:
            (recon, mse) = model(video[::8][:log_samples], tau, args.hard)
            N, T, C, H, W = recon.size()
            tiles = recon.reshape(N, T*C, H, W) # (N, T*C, H, W)
            frames = vutils.make_grid(tiles, nrow=(N), pad_value=0.8)  # (T*C, H', W')
            frames = frames.reshape(1,T,C,frames.shape[-2],frames.shape[-1])
        ...
        if val_loss < best_val_loss:
            ...
            best_epoch = epoch + 1
            torch.save(state_dict, best_pth)
